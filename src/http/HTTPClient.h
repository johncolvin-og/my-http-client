...
class CHTTPClient {
 public:
   // Public definitions
   typedef std::function<int(void *, double, double, double, double)>
       ProgressFnCallback;
   ...
   struct ProgressFnStruct {
      ...
      double dLastRunTime;
      CURL *pCurl;
      /* owner of the CHTTPClient object. can be used in the body of the
       * progress function to send signals to the owner (e.g. to update a GUI's
       * progress bar)
       */
      void *pOwner;
   };
   ...
   /* Please provide your logger thread-safe routine, otherwise, you can turn
    * off error log messages printing by not using the flag ALL_FLAGS or
    * ENABLE_LOG */
   ...
   /*inline*/ void SetProgressFnCallback(
   ...
   inline auto GetProgressFnCallback() const {
      return m_fnProgressCallback
          .target<int (*)(void *, double, double, double, double)>();
   }
   inline void *GetProgressFnCallbackOwner() const {
      return m_ProgressStruct.pOwner;
   }
   ...
   // HTTP requests
   const bool GetText(
       const std::string &strURL, std::string &strText, long &lHTTPStatusCode);
   ...
   // SSL certs
   static const std::string &GetCertificateFile() {
      return s_strCertificationAuthorityFile;
   }
   static void SetCertificateFile(const std::string &strPath) {
      s_strCertificationAuthorityFile = strPath;
   }

   void SetSSLCertFile(const std::string &strPath) {
      m_strSSLCertFile = strPath;
   }
   ...
   void SetSSLKeyPassword(const std::string &strPwd) {
      m_strSSLKeyPwd = strPwd;
   }
 ...
 protected:
   ...
   inline const bool InitRestRequest(
       const std::string &strUrl,
       const HeadersMap &Headers,
       HttpResponse &Response);
   inline const bool PostRestRequest(
       const CURLcode ePerformCode, HttpResponse &Response);

   // Curl callbacks
   static size_t WriteInStringCallback(
       void *ptr, size_t size, size_t nmemb, void *data);
   static size_t WriteToFileCallback(
       void *ptr, size_t size, size_t nmemb, void *data);
   static size_t WriteToMemoryCallback(
       void *ptr, size_t size, size_t nmemb, void *data);
   static size_t ReadFromFileCallback(
       void *ptr, size_t size, size_t nmemb, void *stream);
   static size_t ThrowAwayCallback(
       void *ptr, size_t size, size_t nmemb, void *data);
   static size_t RestWriteCallback(
       void *ptr, size_t size, size_t nmemb, void *userdata);
   static size_t RestHeaderCallback(
       void *ptr, size_t size, size_t nmemb, void *userdata);
   static size_t RestReadCallback(
       void *ptr, size_t size, size_t nmemb, void *userdata);
...
inline CHTTPClient::SettingsFlag operator|(
    CHTTPClient::SettingsFlag a, CHTTPClient::SettingsFlag b) {
   return static_cast<CHTTPClient::SettingsFlag>(
...
#define LOG_WARNING_OBJECT_NOT_CLEANED                                         \
   "[HTTPClient][Warning] Object was freed before calling "                    \
   "CHTTPClient::CleanupSession(). The API session was cleaned though."
#define LOG_ERROR_CURL_ALREADY_INIT_MSG                                        \
   "[HTTPClient][Error] Curl session is already initialized ! "                \
   "Use CleanupSession() to clean the present one."
#define LOG_ERROR_CURL_NOT_INIT_MSG                                            \
   "[HTTPClient][Error] Curl session is not initialized ! Use InitSession() "  \
   "before."

#define LOG_ERROR_CURL_REQ_FAILURE_FORMAT                                      \
   "[HTTPClient][Error] Unable to perform request from '%s' "                  \
   "(Error = %d | %s) (HTTP_Status = %ld)"
#define LOG_ERROR_CURL_REST_FAILURE_FORMAT                                     \
   "[HTTPClient][Error] Unable to perform a REST request from '%s' "           \
   "(Error = %d | %s)"
#define LOG_ERROR_CURL_DOWNLOAD_FAILURE_FORMAT                                 \
   "[HTTPClient][Error] Unable to perform a request - '%s' from '%s' "         \
   "(Error = %d | %s) (HTTP_Status = %ld)"
#define LOG_ERROR_DOWNLOAD_FILE_FORMAT                                         \
   "[HTTPClient][Error] Unable to open local file %s"

class HTTPClient : public CHTTPClient {
 public:
   static void really() {
   ...
   HTTPClient()
       : CHTTPClient([](const std::string &log_msg) {
            std::cout << log_msg << std::endl;
         }) {}
   ~HTTPClient() = default;
};
