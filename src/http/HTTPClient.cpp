...
CHTTPClient::CHTTPClient(LogFnCallback Logger)
    : m_bNoSignal(false), m_bHTTPS(false), m_eSettingsFlags(ALL_FLAGS),
      m_pHeaderlist(nullptr), m_pCurlSession(nullptr), m_iCurlTimeout(0),
      m_bProgressCallbackSet(false), m_oLog(Logger),
 ...
 * Use CleanupSession() before initializing a new one or the Curl API is not
 * initialized.
 *
 * Example Usage:
 * @code
 ...
 * @endcode
 */
const bool CHTTPClient::InitSession(
    const bool &bHTTPS /* = false */,
 ...
 *
 */
/*inline*/ void CHTTPClient::SetProgressFnCallback(
    void *pOwner, const ProgressFnCallback &fnCallback) {
...
const CURLcode CHTTPClient::Perform() {
   ...
   if (m_bProgressCallbackSet) {
      curl_easy_setopt(
          m_pCurlSession, CURLOPT_PROGRESSFUNCTION, *GetProgressFnCallback());
   ...
   if (m_bHTTPS) {
      ...
      curl_easy_setopt(
          m_pCurlSession, CURLOPT_SSL_VERIFYPEER,
          (m_eSettingsFlags & VERIFY_PEER) ? 1L : 0L);
      curl_easy_setopt(
          m_pCurlSession, CURLOPT_SSL_VERIFYPEER,
          (m_eSettingsFlags & CURLOPT_SSL_VERIFYHOST) ? 2L : 0L);
   }

   if (m_bHTTPS && !s_strCertificationAuthorityFile.empty())
      curl_easy_setopt(
          m_pCurlSession, CURLOPT_CAINFO,
          s_strCertificationAuthorityFile.c_str());

   if (m_bHTTPS && !m_strSSLCertFile.empty())
      curl_easy_setopt(
          m_pCurlSession, CURLOPT_SSLCERT, m_strSSLCertFile.c_str());
   ...
   if (m_bHTTPS && !m_strSSLKeyPwd.empty())
      curl_easy_setopt(
          m_pCurlSession, CURLOPT_KEYPASSWD, m_strSSLKeyPwd.c_str());
 ...
 *
 * @param [in] strURL URI of the remote location (with the file name) encoded in
 * UTF-8 format.
 ...
 *    m_pHTTOClient->GetText("https://www.google.com", strWebPage,
 * lHTTPStatusCode);
 * @endcode
 */
const bool CHTTPClient::GetText(
    const std::string &strURL, std::string &strOutput, long &lHTTPStatusCode) {
   if (strURL.empty()) {
      if (m_eSettingsFlags & ENABLE_LOG)
   ...
   curl_easy_setopt(
       m_pCurlSession, CURLOPT_WRITEFUNCTION, WriteInStringCallback);
   ...
   if (res != CURLE_OK) {
      if (m_eSettingsFlags & ENABLE_LOG)
         m_oLog(StringFormat(
             LOG_ERROR_CURL_REQ_FAILURE_FORMAT, m_strURL.c_str(), res,
 ...
 *
 * @param [in] strLocalFile Complete path of the local file to download in UTF-8
 * format.
 * @param [in] strURL URI of the remote location (with the file name) encoded in
 * UTF-8 format.
 * @param [out] lHTTPStatusCode HTTP Status code of the response.
 ...
 * more information.
 */
const bool CHTTPClient::DownloadFile(
   ...
   if (ofsOutput) {
      curl_easy_setopt(m_pCurlSession, CURLOPT_HTTPGET, 1L);
      curl_easy_setopt(
          m_pCurlSession, CURLOPT_WRITEFUNCTION, WriteToFileCallback);
      ...
      ofsOutput.close();
      curl_easy_getinfo(
          m_pCurlSession, CURLINFO_RESPONSE_CODE, &lHTTPStatusCode);

      // double dUploadLength = 0;
      // curl_easy_getinfo(m_pCurlSession, CURLINFO_CONTENT_LENGTH_UPLOAD,
      // &dUploadLength); // number of bytes uploaded

      /* Delete downloaded file if status code != 200 as server's response body
      may contain error 404 */
      ...
      if (res != CURLE_OK) {
         if (m_eSettingsFlags & ENABLE_LOG)
            m_oLog(StringFormat(
                LOG_ERROR_CURL_DOWNLOAD_FAILURE_FORMAT, strLocalFile.c_str(),
      ...
      m_oLog(
          StringFormat(LOG_ERROR_DOWNLOAD_FILE_FORMAT, strLocalFile.c_str()));
...
const bool CHTTPClient::DownloadFile(
   ...
   curl_easy_setopt(m_pCurlSession, CURLOPT_HTTPGET, 1L);
   curl_easy_setopt(
       m_pCurlSession, CURLOPT_WRITEFUNCTION, WriteToMemoryCallback);
   ...
   if (res != CURLE_OK) {
      if (m_eSettingsFlags & ENABLE_LOG)
         m_oLog(StringFormat(
             LOG_ERROR_CURL_DOWNLOAD_FAILURE_FORMAT,
             "Download to a byte buffer", strURL.c_str(), res,
             curl_easy_strerror(res), lHTTPStatusCode));
 ...
 *
 *
 * @param [in] strURL URL to which the form will be posted encoded in UTF-8
 * format.
 ...
 */
const bool CHTTPClient::UploadForm(
    const std::string &strURL,
 ...
 */
void CHTTPClient::PostFormInfo::AddFormFile(
    const std::string &strFieldName, const std::string &strFieldValue) {
   curl_formadd(
       &m_pFormPost, &m_pLastFormptr, CURLFORM_COPYNAME, strFieldName.c_str(),
       CURLFORM_FILE, strFieldValue.c_str(), CURLFORM_END);
}
 ...
 *
 * @param fieldName name of the input element encoded in UTF8 for Linux and in
 * ANSI for Windows (so the file gets located and uploaded).
 * @param fieldValue value to be assigned to the input element encoded in UTF8
 * for Linux and in ANSI for Windows.
 */
void CHTTPClient::PostFormInfo::AddFormContent(
    const std::string &strFieldName, const std::string &strFieldValue) {
   curl_formadd(
       &m_pFormPost, &m_pLastFormptr, CURLFORM_COPYNAME, strFieldName.c_str(),
       CURLFORM_COPYCONTENTS, strFieldValue.c_str(), CURLFORM_END);
}
 ...
 */
inline const bool CHTTPClient::InitRestRequest(
    const std::string &strUrl,
   ...
   curl_easy_setopt(
       m_pCurlSession, CURLOPT_WRITEFUNCTION, &CHTTPClient::RestWriteCallback);
   ...
   curl_easy_setopt(
       m_pCurlSession, CURLOPT_HEADERFUNCTION,
       &CHTTPClient::RestHeaderCallback);

   // callback object for server's responses headers
   curl_easy_setopt(m_pCurlSession, CURLOPT_HEADERDATA, &Response);

   std::string strHeader;
   for (HeadersMap::const_iterator it = Headers.cbegin(); it != Headers.cend();
        ++it) {
 ...
 * @param [out] Response response data
 */
inline const bool CHTTPClient::PostRestRequest(
    const CURLcode ePerformCode, CHTTPClient::HttpResponse &Response) {
   // Check for errors
   if (ePerformCode != CURLE_OK) {
      ...
      if (m_eSettingsFlags & ENABLE_LOG)
         m_oLog(StringFormat(
             LOG_ERROR_CURL_REST_FAILURE_FORMAT, m_strURL.c_str(), ePerformCode,
...
const bool CHTTPClient::Post(
   ...
   if (InitRestRequest(strUrl, Headers, Response)) {
      ...
      curl_easy_setopt(
          m_pCurlSession, CURLOPT_POSTFIELDSIZE, strPostData.size());
 ...
 * @retval false  Encountered a problem.
 */
const bool CHTTPClient::Put(
    const std::string &strUrl,
    const CHTTPClient::HeadersMap &Headers,
    const std::string &strPutData,
    CHTTPClient::HttpResponse &Response) {
   if (InitRestRequest(strUrl, Headers, Response)) {
      ...
      curl_easy_setopt(
          m_pCurlSession, CURLOPT_READFUNCTION, &CHTTPClient::RestReadCallback);
      // set data object to pass to callback function
      curl_easy_setopt(m_pCurlSession, CURLOPT_READDATA, &Payload);

      // set data size
      curl_easy_setopt(
          m_pCurlSession, CURLOPT_INFILESIZE,
          static_cast<long>(Payload.usLength));
...
const bool CHTTPClient::Put(
    const std::string &strUrl,
    const CHTTPClient::HeadersMap &Headers,
    const CHTTPClient::ByteBuffer &Data,
    CHTTPClient::HttpResponse &Response) {
   if (InitRestRequest(strUrl, Headers, Response)) {
      CHTTPClient::UploadObject Payload;
...
inline void CHTTPClient::TrimSpaces(std::string &str) {
   ...
   str.erase(
       std::find_if(
           str.rbegin(), str.rend(), [](char c) { return !isspace(c); })
           .base(),
       str.end());
}
...
size_t CHTTPClient::ThrowAwayCallback(
 ...
 * @return (size * nmemb)
 */
size_t CHTTPClient::WriteInStringCallback(
    void *ptr, size_t size, size_t nmemb, void *data) {
 ...
 * @return (size * nmemb)
 */
size_t CHTTPClient::WriteToFileCallback(
    void *buff, size_t size, size_t nmemb, void *data) {
 ...
 * @return (size * nmemb)
 */
size_t CHTTPClient::WriteToMemoryCallback(
    void *buff, size_t size, size_t nmemb, void *data) {
   ...
   std::copy(
       ...
       reinterpret_cast<unsigned char *>(buff) + ssize,
 ...
 * @return (size * nmemb)
 */
size_t CHTTPClient::ReadFromFileCallback(
 ...
 *
 * we can also use an std::vector<char> instead of an std::string but in this
 * case there isn't a big difference... maybe resizing the container with a max
 * size can enhance performances...
 *
 ...
 * @return (size * nmemb)
 */
size_t CHTTPClient::RestWriteCallback(
    void *pCurlData, size_t usBlockCount, size_t usBlockSize, void *pUserData) {
   CHTTPClient::HttpResponse *pServerResponse;
   pServerResponse = reinterpret_cast<CHTTPClient::HttpResponse *>(pUserData);
   pServerResponse->strBody.append(
       reinterpret_cast<char *>(pCurlData), usBlockCount * usBlockSize);
 ...
 * @return size * nmemb;
 */
size_t CHTTPClient::RestHeaderCallback(
    void *pCurlData, size_t usBlockCount, size_t usBlockSize, void *pUserData) {
   CHTTPClient::HttpResponse *pServerResponse;
   pServerResponse = reinterpret_cast<CHTTPClient::HttpResponse *>(pUserData);
 ...
 *
 * @param pointer of max size (size*nmemb) to write data to (used by cURL to
 * send data)
 * @param size size parameter
 * @param nmemb memblock parameter
 ...
 * @return (size * nmemb)
 */
size_t CHTTPClient::RestReadCallback(
    void *pCurlData, size_t usBlockCount, size_t usBlockSize, void *pUserData) {
   // get upload struct
   CHTTPClient::UploadObject *Payload;
   ...
   size_t usCopySize =
       (Payload->usLength < usCurlSize) ? Payload->usLength : usCurlSize;
...
#ifdef WINDOWS
       && s_strCurlTraceLogDirectory.at(
              s_strCurlTraceLogDirectory.length() - 1) != '\\') {
...
void CHTTPClient::StartCurlDebug() const {
   if (!m_ofFileCurlTrace.is_open()) {
      ...
      if (!strFileCurlTraceFullName.empty()) {
         ...
         m_ofFileCurlTrace.open(
             strFileCurlTraceFullName,
             std::ifstream::app | std::ifstream::binary);

         if (m_ofFileCurlTrace)
            curl_easy_setopt(
                m_pCurlSession, CURLOPT_DEBUGDATA, &m_ofFileCurlTrace);
      }
   }
}
...
std::string CHTTPClient::AnsiToUtf8(const std::string &codepage_str) {
   ...
   int size = MultiByteToWideChar(
       CP_ACP, MB_PRECOMPOSED, codepage_str.c_str(), codepage_str.length(),
       nullptr, 0);
   ...
   int utf8_size = WideCharToMultiByte(
       CP_UTF8, 0, utf16_str.c_str(), utf16_str.length(), nullptr, 0, nullptr,
       nullptr);
   std::string utf8_str(utf8_size, '\0');
   WideCharToMultiByte(
       CP_UTF8, 0, utf16_str.c_str(), utf16_str.length(), &utf8_str[0],
